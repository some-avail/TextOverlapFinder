...
-n: or --exclude_noisewords; example: -n:english_generic

Since 2.36 introduced, one can use this option with batch-operations
(not yet available for singular ops). 
It filters the resulting matches to exclude phrases with noise-words. 
Noise-words are words like: the, have, are, been etc.
Currently only phrases are excluded that have only noise-words in them.
Noise-word-files have the format: noise_words_*.dat where * stands 
for a specifier indicating language and subject.
The specifier for english is english_generic so that the file
noise_words_english_generic.dat is used as source.

I provide currently 3 noise-word-files; others can be generated by 
using my application Freekwensies, findable on my github-page.
